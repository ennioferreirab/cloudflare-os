...
describe("ObservationGate", () => {
  ...
  it("releases the fence when the latch write fails", async () => {
    // The count outlives the tracker, so a leak here denies admission for the life of the object.
    // Nothing was disclosed -- the read throws with the write -- so nothing is owed a fence.
    const kv = makeKv();
  ...
  it("releases the fence when enumerating observers fails", async () => {
    const kv = makeKv();
    let scan = true;
    const failing: ObserverKv = {
      ...kv,
