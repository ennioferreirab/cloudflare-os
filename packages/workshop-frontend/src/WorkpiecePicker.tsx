  ...
  return (
    ...
    >
      <button
        ...
        title={expanded ? t('workspace.picker.collapseOutputs') : t('workspace.picker.expandOutputs')}
        aria-label={expanded ? t('workspace.picker.collapseOutputs') : t('workspace.picker.expandOutputs')}
      ...
      >
        {expanded && (
          <span className="text-[11px] font-medium uppercase tracking-[0.06em]">{t('workspace.picker.outputs')}</span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
            ...
            return (
              <div key={gadget.id} className="flex items-center gap-1 py-0.5">
                ...
                <WorkshopIconButton
                  ...
                  className="!h-6 !w-6"
                  aria-label={t('workspace.picker.saveGadgetName')}
                >
                  <Check size={13} />
                </WorkshopIconButton>
                <WorkshopIconButton
                  ...
                  className="!h-6 !w-6"
                  aria-label={t('workspace.picker.cancelRename')}
                >
                  <X size={13} />
                </WorkshopIconButton>
              </div>
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? t('workspace.picker.draftSuffix') : ''}${hasHook ? t('workspace.picker.hooksSuffix') : ''}`} asChild>
                <button
                  type="button"
                ...
                >
                  ...
                  {hasHook && (
                    <span
                      role="img"
                      aria-label={t('workspace.picker.hooksEnabled')}
                      className={expanded
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  title={t('workspace.picker.renameGadget')}
                  aria-label={t('workspace.picker.renameGadgetNamed', { name: gadget.title })}
                >
        ...
        <Tooltip content={t('workspace.picker.viewActivity')} asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">{t('workspace.picker.viewActivity')}</span>}
            {expanded ? (
