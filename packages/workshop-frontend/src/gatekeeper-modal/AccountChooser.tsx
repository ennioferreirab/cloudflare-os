  ...
  return (
    <section className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
      <div className="border-b border-kumo-line px-3 py-2.5">
        <p className="text-[12px] leading-4 font-medium tracking-[-0.2px] text-kumo-default">{t('connections.account.heading')}</p>
        <p className="mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
          {isEmailMailbox
            ? t('connections.account.emailMailboxDescription')
            : t('connections.account.chooseIdentity', {
                vendor: vendorName,
                resource: resourceTitle ?? t('connections.modal.createConnection'),
              })}
        </p>
      </div>
      <div className="divide-y divide-kumo-line">
        {accounts.map(account => {
          ...
          const name = account.description.uniqueName || account.description.displayName || t('connections.account.connectedAccountFallback')
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`truncate text-[12px] leading-4 font-normal tracking-[-0.2px] ${needsAccess ? 'text-kumo-brand' : 'text-kumo-subtle'}`}>
                    {expired
                      ? t('connections.account.expiredCredentials')
                      : needsAccess
                      ? t('connections.account.additionalPermissionNeeded')
                      : resourceTitle
                      ? t('connections.account.connectedVendorAccount', { vendor: vendorName })
              ...
              {expired ? (
                ...
                >
                  {reconnecting ? t('connections.account.opening') : t('connections.account.reconnect')}
                </button>
              ) : needsAccess ? (
                <button
                  type="button"
                  ...
                  className="shrink-0 cursor-pointer rounded-md border border-kumo-line px-2 py-1 text-[12px] leading-4 font-medium tracking-[-0.2px] text-kumo-default transition-colors hover:bg-kumo-elevated disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {granting ? t('connections.account.opening') : t('connections.account.grantAccess')}
                </button>
              ) : null}
        ...
        {(!isEmailMailbox || accounts.length === 0) && (
          ...
          >
            ...
            {isEmailMailbox
              ? t('connections.account.enableEmailMailboxes')
              : accounts.length === 0
              ? t('connections.account.connectVendor', { vendor: vendorName })
              : t('connections.account.useAnotherVendor', { vendor: vendorName })}
          </button>
        )}
      </div>
    </section>
