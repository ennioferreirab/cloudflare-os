...
function BlueprintsRoutePage() {
  const { t } = useLocale()
  useDocumentTitle(t('library.blueprints.title'))
  return (
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-3 sm:px-10">
      ...
      <header className="min-w-0 px-3 pb-3 pt-6 sm:pt-10">
        <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t('library.blueprints.title')}</h1>
        <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
          {t('library.blueprints.description')}
        </p>
      </header>
