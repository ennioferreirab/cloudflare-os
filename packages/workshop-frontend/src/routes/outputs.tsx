...
function formatRelativeTime(date: Date, t: ReturnType<typeof useLocale>['t']): string {
  ...
  if (minutes < 1) return t('library.outputs.time.justNow')
  if (minutes < 60) return t('library.outputs.time.minutesAgo', { count: minutes })
  const hours = Math.floor(minutes / 60)
  if (hours < 24) return t('library.outputs.time.hoursAgo', { count: hours })
  const days = Math.floor(hours / 24)
  return t('library.outputs.time.daysAgo', { count: days })
}
      ...
      <DropdownMenu>
        <DropdownMenu.Trigger
          render={
            <button
              type="button"
              aria-label={t('library.outputs.actions')}
        ...
        <DropdownMenu.Content className={MENU_CONTENT}>
          <DropdownMenu.Item onClick={onOpen} className={MENU_ITEM}>
            <ArrowSquareOut size={13} className="mr-2" /> {t('library.outputs.open')}
          </DropdownMenu.Item>
          <DropdownMenu.Item onClick={onOpenWorkspace} className={MENU_ITEM}>
            <Cube size={13} className="mr-2" /> {t('library.outputs.openWorkspace')}
          </DropdownMenu.Item>
          {onRename && (
            <DropdownMenu.Item onClick={onRename} className={MENU_ITEM}>
              <PencilSimple size={13} className="mr-2" /> {t('library.outputs.rename')}
            </DropdownMenu.Item>
          )}
          {onRemove && (
            <DropdownMenu.Item onClick={onRemove} className={`${MENU_ITEM} text-kumo-danger`}>
              <Trash size={13} className="mr-2" /> {t('library.outputs.remove')}
            </DropdownMenu.Item>
          )}
...
function subtitle(output: OutputSummary, t: ReturnType<typeof useLocale>['t']): string {
  const parts = [output.workspaceTitle || t('library.outputs.untitledWorkspace')]
  if (output.owner) parts.push(t('library.outputs.sharedBy', { name: output.owner.name }))
  parts.push(t('library.outputs.workspaceActive', { time: formatRelativeTime(output.lastActive, t) }))
  return parts.join(' · ')
}
...
function OutputProvenance({ owner }: { owner?: OutputSummary['owner'] }) {
  ...
  return (
    <span
      ...
      title={owner ? t('library.outputs.inWorkspaceSharedBy', { name: owner.name }) : t('library.outputs.inWorkspaceCreatedByYou')}
    >
      {owner ? <ShareNetwork size={11} /> : <User size={11} />}
      <span className="truncate">{owner ? t('library.outputs.sharedBy', { name: owner.name }) : t('library.outputs.createdByYou')}</span>
    </span>
  )
}
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-medium leading-[18px] tracking-[-0.25px] text-kumo-default">
            {output.title || t('library.outputs.untitled')}
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
          {formatOf(output.output).noun} · {output.workspaceTitle || t('library.outputs.untitledWorkspace')}
        </p>
      </div>
      ...
      <div className="hidden shrink-0 items-center gap-6 text-xs text-kumo-inactive lg:flex">
        ...
        <span className="flex w-40 items-center justify-end gap-1 whitespace-nowrap">
          <Clock size={10} />
          {t('library.outputs.workspaceActive', { time: formatRelativeTime(output.lastActive, t) })}
        </span>
      </div>
      <OutputMenu onOpen={onOpen} onOpenWorkspace={onOpenWorkspace}
                  onRename={onRename} onRemove={onRemove} />
...
function ScopeSelect({
  ...
  const options: { value: OwnerFilter; label: string }[] = [
    { value: 'all', label: t('library.outputs.yoursAndShared') },
    { value: 'mine', label: t('library.outputs.createdByYou') },
    { value: 'shared', label: t('library.outputs.sharedWithYou') },
  ]
        ...
        <form onSubmit={(event) => { event.preventDefault(); onSave() }}>
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 text-kumo-subtle">
                {t('library.outputs.renameDescription', { workspace: output?.workspaceTitle ?? '' })}
              </Dialog.Description>
            </div>
            <WorkshopIconButton type="button" className="!h-7 !w-7" disabled={busy} aria-label={t('library.outputs.close')} onClick={onClose}>
              <X size={16} />
            </WorkshopIconButton>
          </div>
          <div className="px-5 py-4">
            <label className="block text-[12px] font-medium text-kumo-subtle" htmlFor="rename-output-title">
              {t('library.outputs.name')}
            </label>
            <input
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line px-5 py-3">
            ...
            <WorkshopButton tone="primary" type="submit" disabled={busy || !value.trim()}>
              {busy ? t('library.outputs.saving') : t('library.outputs.save')}
            </WorkshopButton>
          </div>
        </form>
      </Dialog>
...
function OutputsPage() {
  const { t } = useLocale()
  useDocumentTitle(t('library.outputs.title'))
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (loadedOnce.current) {
        toastsRef.current.add({ title: t('library.outputs.refreshFailed'), variant: 'error' })
      } else {
        setLoadError(true)
      }
    })
  ...
  const saveRename = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('library.outputs.renameFailed'), variant: 'error' })
    } finally {
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('library.outputs.removeFailed'), variant: 'error' })
    } finally {
      gadget?.[Symbol.dispose]()
      overseer?.[Symbol.dispose]()
      setMutationBusy(false)
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col px-3 sm:px-10">
      <header className="flex items-end justify-between gap-4 px-3 pb-4 pt-6 sm:pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t('library.outputs.title')}</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            {t('library.outputs.description')}
          </p>
        </div>
      ...
      <div className={`flex flex-col gap-3 px-3 pb-3 sm:flex-row sm:items-center sm:justify-between ${
        ...
        <div className="-mx-1 flex items-center gap-1 overflow-x-auto px-1 sidebar-scroll">
          {showTypeFilters && (
            <>
              <FilterChip active={typeFilter === 'all'} label={t('library.outputs.all')} count={inTypeScope.length}
        ...
        <div className="flex min-w-0 items-center gap-2 sm:shrink-0">
          ...
          <div className="relative min-w-0 flex-1 sm:w-56 sm:flex-none">
            ...
            <input
              ...
              placeholder={t('library.outputs.searchPlaceholder')}
      ...
      <div className="chat-panel min-h-0 flex-1 overflow-y-auto pb-8 pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">{t('library.outputs.loadFailed')}</p>
            <button onClick={() => setReloadToken((n) => n + 1)} className="mt-1 text-kumo-brand underline">
              {t('library.outputs.retry')}
            </button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 px-3 py-20 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">
                {isFiltered ? t('library.outputs.noMatch') : t('library.outputs.noOutputs')}
              </p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {isFiltered
                  ? t('library.outputs.filteredHint')
                  : t('library.outputs.emptyHint')}
              </p>
            </div>
            {/* Offer the deployment's formats here rather than sending them to the home page. */}
            {!isFiltered && <NewFormatRow label={t('library.outputs.startWith')} />}
          </div>
        ) : view === 'grid' ? (
          <div className="grid grid-cols-1 gap-4 px-3 min-[400px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((output) => (
      ...
      <DeleteConfirmationDialog
        ...
        title={t('library.outputs.removeTitle', { title: removeOutput?.title || t('library.outputs.untitled') })}
        description={
          <>
            {t('library.outputs.removeDescription', {
              workspace: removeOutput?.workspaceTitle ?? '',
              scope: removeOutput?.owner ? t('library.outputs.everyoneWithAccess') : '',
            })}
          </>
        }
        confirmLabel={t('library.outputs.remove')}
        confirmingLabel={t('library.outputs.removing')}
