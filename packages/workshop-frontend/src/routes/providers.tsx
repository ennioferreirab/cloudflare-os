...
function ModelRow({
  ...
  return (
    <div
      ...
      title={isQuick ? t('library.providers.quickModelTitle') : t('library.providers.setQuickModelTitle')}
      ...
      <div onClick={(e) => { e.stopPropagation() }}>
        <DropdownMenu>
          <DropdownMenu.Trigger
            render={
              <button
                aria-label={t('library.providers.providerActions')}
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={onSetQuick} className={MENU_ITEM}>
              ...
              {isQuick ? t('library.providers.clearQuickModel') : t('library.providers.setQuickModel')}
            </DropdownMenu.Item>
            {!isBuiltIn && (
              <DropdownMenu.Item variant="danger" onClick={onDelete} className={MENU_ITEM_DANGER}>
                <Trash size={13} className="mr-2" />
                {t('library.providers.deleteProvider')}
              </DropdownMenu.Item>
            )}
...
function ProvidersPage() {
  const { t } = useLocale()
  useDocumentTitle(t('library.providers.title'))
  ...
  const handleDelete = async (model: AiChatAuthorInfo) => {
    if (!confirm(t('library.providers.deleteConfirmation', { name: model.name }))) return
    ...
    try {
      ...
      toasts.add({ title: t('library.providers.deleteFailed'), variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const handleSetQuick = async (modelId: string) => {
    ...
    try {
      ...
      toasts.add({ title: t('library.providers.defaultModelFailed'), variant: 'error' })
    } finally {
    ...
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-3 sm:px-10">
      <header className="flex flex-col items-stretch gap-4 px-3 pb-3 pt-6 sm:flex-row sm:items-end sm:justify-between sm:pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t('library.providers.title')}</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            {t('library.providers.description')}
          </p>
        </div>
        ...
        <div className="mb-3 px-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('library.providers.searchPlaceholder')}
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1 pb-16">
          ...
          <div className="flex flex-col gap-2.5 px-3 pb-2">
            {gatewayMode && (
              <Notice>
                ...
                <span>
                  <strong className="font-medium text-kumo-default">{t('library.providers.gatewayModeLabel')}</strong>{' '}
                  {t('library.providers.gatewayModeDescription')}
                </span>
              </Notice>
            )}
            ...
            {!gatewayMode && models.length > 0 && (
              <Notice>
                <Lightning size={15} className="mt-px shrink-0 text-kumo-brand" />
                <span>
                  <strong className="font-medium text-kumo-default">{t('library.providers.quickModelLabel')}</strong>{' '}
                  {quickModel
                    ? `${models.find((m) => m.id === quickModel)?.name ?? quickModel}.`
                    : t('library.providers.quickModelNone')}{' '}
                  {t('library.providers.quickModelDescription')}
                </span>
              </Notice>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">{t('library.providers.noProviders')}</p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {t('library.providers.noProvidersDescription')}
              </p>
            </div>
            <button type="button" onClick={() => setSheetOpen(true)} className={PRIMARY_BTN}>
              <Plus size={14} weight="bold" />
              {t('library.providers.addFirstProvider')}
            </button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-12 text-center text-sm text-kumo-inactive">{t('library.providers.noMatch')}</div>
        ) : (
          filtered.map((model) => (
            <div
              key={model.id}
