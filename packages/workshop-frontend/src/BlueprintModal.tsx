...
export default function BlueprintModal({ open, onClose, overseer, gadget, metadata }: Props) {
  ...
  const loadBlueprints = useCallback(async () => {
    ...
    try {
      ...
      toasts.add({ title: t('library.blueprintModal.loadFailed'), variant: 'error' })
    } finally {
      setLoading(false)
    }
  }, [overseer, t])
  ...
  const loadBindings = useCallback(async () => {
    ...
    try {
      ...
      setBindingsError(t('library.blueprintModal.connectionsLoadFailed'))
    } finally {
  ...
  const handleScreenshotSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      toasts.add({ title: t('library.blueprintModal.imageFileRequired'), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t('library.blueprintModal.screenshotProcessFailed'), variant: 'error' })
    } finally {
  ...
  const createBlueprint = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('library.blueprintModal.created'), variant: 'success' })
      setFormMode('list')
      setNewTitle(metadata.title)
      setNewDescription('')
      setNewScreenshotBlob(null)
      ...
      setCreateError(err.message || t('library.blueprintModal.createFailed'))
    } finally {
      setCreating(false)
    }
  }
  ...
  const saveBlueprintEdits = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('library.blueprintModal.updated'), variant: 'success' })
      setFormMode('list')
      ...
      setCreateError(err.message || t('library.blueprintModal.updateFailed'))
    } finally {
      setCreating(false)
    }
  }
  ...
  const deleteBlueprint = async (id: string) => {
    ...
    try {
      ...
      toasts.add({ title: t('library.blueprintModal.deleted'), variant: 'success' })
      setConfirmingDeleteId(null)
      await loadBlueprints()
    } catch (err: any) {
      toasts.add({ title: err.message || t('library.blueprintModal.deleteFailed'), variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
          ...
          <div className="flex shrink-0 items-start justify-between gap-4 border-b border-kumo-line px-4 py-5 sm:px-6">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <Dialog.Title className="text-[17px] leading-6 font-medium tracking-[-0.35px] text-kumo-default">
                {formMode === 'create' ? t('library.blueprintModal.createTitle') : formMode === 'edit' ? t('library.blueprintModal.editTitle') : t('library.blueprintModal.listTitle')}
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {formMode === 'create'
                  ? t('library.blueprintModal.createDescription')
                  : formMode === 'edit'
                    ? t('library.blueprintModal.editDescription')
                    : t('library.blueprintModal.listDescription')}
              </Dialog.Description>
              </div>
            </div>
            <Dialog.Close
              render={(props) => (
                <WorkshopIconButton
                  {...props}
                  aria-label={t('library.blueprintModal.close')}
                >
                  <X size={18} />
                </WorkshopIconButton>
              )}
          ...
          <div className="min-h-0 flex-1">
              ...
              <div className="flex h-full min-h-0 flex-col">
                <div className="chat-panel min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-5 sm:px-6">
                  <div className="space-y-3">
                    <WorkshopInput
                      placeholder={t('library.blueprintModal.titlePlaceholder')}
                      aria-label={t('library.blueprintModal.titleAriaLabel')}
                      value={newTitle}
                      onChange={(e) => setNewTitle(e.target.value)}
                      className="w-full"
                    />
                    <WorkshopInputArea
                      placeholder={t('library.blueprintModal.descriptionPlaceholder')}
                      aria-label={t('library.blueprintModal.descriptionAriaLabel')}
                    ...
                    <div className="rounded-xl border border-kumo-line bg-kumo-base p-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <p className="m-0 text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            {t('library.blueprintModal.screenshot')}
                          </p>
                          <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            {t('library.blueprintModal.screenshotDescription')}
                            {formMode === 'edit' && !newScreenshotUrl && editingBlueprint?.screenshotUrl && !clearScreenshot ? ` ${t('library.blueprintModal.screenshotKeepDescription')}` : ''}
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-1.5">
                          {(newScreenshotUrl || (formMode === 'edit' && editingBlueprint?.screenshotUrl && !clearScreenshot)) && (
                            ...
                            >
                              {t('library.blueprintModal.clear')}
                            </WorkshopButton>
                          )}
                          <WorkshopButton
                            className="!h-8"
                          ...
                          >
                            ...
                            {processingScreenshot ? t('library.blueprintModal.processing') : newScreenshotUrl || (formMode === 'edit' && editingBlueprint?.screenshotUrl && !clearScreenshot) ? t('library.blueprintModal.change') : t('library.blueprintModal.upload')}
                          </WorkshopButton>
                        </div>
                      </div>
                      {screenshotPreviewUrl && (
                        <div className="mt-3 overflow-hidden rounded-lg border border-kumo-line bg-kumo-tint">
                          <img
                            ...
                            alt={t('library.blueprintModal.screenshotPreview')}
                      ...
                      {clearScreenshot && !newScreenshotUrl && (
                        <div className="mt-3 rounded-lg border border-dashed border-kumo-line bg-kumo-tint px-3 py-2 text-[12px] leading-4 text-kumo-subtle">
                          {t('library.blueprintModal.screenshotWillRemove')}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {bindingsLoading ? (
                    <div className="rounded-xl border border-kumo-line bg-kumo-base px-4 py-6 text-center text-[13px] text-kumo-subtle">
                      {t('library.blueprintModal.loadingConnections')}
                    </div>
                  ) : bindingsError ? (
                ...
                <div className="shrink-0 border-t border-kumo-line px-4 py-4 sm:px-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {creating
                        ? formMode === 'create' ? t('library.blueprintModal.creating') : t('library.blueprintModal.saving')
                        : processingScreenshot ? t('library.blueprintModal.processing')
                          : bindingsLoading ? t('library.blueprintModal.loading')
                            : formMode === 'create' ? t('library.blueprintModal.create') : t('library.blueprintModal.save')}
                    </WorkshopButton>
                  </div>
                </div>
              </div>
              ...
              >
                <span>
                  <span className="block text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                    {t('library.blueprintModal.publishBlueprintTitle')}
                  </span>
                  <span className="mt-0.5 block text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                    {t('library.blueprintModal.publishBlueprintDescription')}
                  </span>
                </span>
            ...
            <section>
              <h3 className="mb-2 text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                {t('library.blueprintModal.existingBlueprints')}
              </h3>

              {loading ? (
                <div className="rounded-xl border border-kumo-line bg-kumo-base px-4 py-6 text-center text-[13px] text-kumo-subtle">
                  {t('library.blueprintModal.loading')}
                </div>
              ) : blueprints.length === 0 ? (
                <div className="rounded-xl border border-dashed border-kumo-line bg-kumo-base px-4 py-6 text-center">
                  <p className="text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                    {t('library.blueprintModal.noBlueprints')}
                  </p>
                </div>
              ) : (
                    ...
                    <BlueprintRow
                      ...
                      onUpdateCode={async () => {
                        try {
                          ...
                          toasts.add({ title: t('library.blueprintModal.updatedToCurrentCode'), variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || t('library.blueprintModal.updateFailed'), variant: 'error' })
                        }
                      }}
                      onRetryPublish={async () => {
                        try {
                          await overseer.retryBlueprintPublish(bp.id)
                          toasts.add({ title: t('library.blueprintModal.publishedSuccessfully'), variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || t('library.blueprintModal.retryFailed'), variant: 'error' })
                        }
                      }}
                      onCopyLink={async () => {
...
function BlueprintRow({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => setCopyState('idle'), 1500)
        ...
        <div className="flex w-full flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            <p className="m-0 truncate text-[14px] leading-5 font-semibold tracking-[-0.3px] text-kumo-danger">
              {t('library.blueprintModal.deleteConfirmation', { title: bp.title })}
            </p>
            <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {t('library.blueprintModal.deleteDescription')}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {isDeleting ? t('library.blueprintModal.deleting') : t('library.blueprintModal.delete')}
          </button>
          <button
            type="button"
          ...
          >
            {t('library.blueprintModal.cancel')}
          </button>
        </div>
      </div>
    )
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-3">
        ...
        <span
          ...
          title={bp.dirty ? t('library.blueprintModal.lastPublishFailed') : undefined}
        >
          {bp.dirty && (
            <span
          ...
          {t('library.blueprintModal.versionDate', { version: bp.version, date: formatDate(new Date(bp.codeVersionDate)) })}
        </span>
      </div>
      ...
      <div className="mt-1.5 min-h-[18px]">
        {bp.description ? (
          ...
          <p className="m-0 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-inactive">
            {t('library.blueprintModal.noDescription')}
          </p>
        )}
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="-ml-[7px] flex flex-wrap items-center gap-1">
          <GhostButton onClick={onUpdateCode} icon={<ArrowsClockwise size={13} />}>
            {t('library.blueprintModal.updateCode')}
          </GhostButton>
          {bp.dirty && (
            <GhostButton onClick={onRetryPublish} icon={<ArrowsClockwise size={13} />}>
              {t('library.blueprintModal.retryPublish')}
            </GhostButton>
          )}
          <GhostButton
            onClick={async () => {
          ...
          >
            {copyState === 'copied' ? t('library.blueprintModal.copied') : copyState === 'failed' ? t('library.blueprintModal.copyFailed') : t('library.blueprintModal.copyLink')}
          </GhostButton>
        </div>
        <div className="-mr-1.5 ml-auto flex items-center gap-0.5 opacity-60 transition-opacity group-hover/row:opacity-100 focus-within:opacity-100">
          <button
            ...
            aria-label={t('library.blueprintModal.editBlueprint')}
          >
            <Pencil size={13} />
          </button>
          <button
            ...
            aria-label={t('library.blueprintModal.deleteBlueprint')}
          >
            <Trash size={13} />
          </button>
        </div>
