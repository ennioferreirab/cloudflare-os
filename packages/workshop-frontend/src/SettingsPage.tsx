...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        if (!cancelled) toasts.add({ title: translateRef.current('profile.loadFailed'), variant: 'error' })
      } finally {
  ...
  const handleSaveName = async () => {
    if (!nameInput.trim()) {
      toasts.add({ title: t('profile.displayNameCannotBeEmpty'), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t('profile.displayNameUpdated'), variant: 'success' })
    } catch (err) {
      console.error('Failed to update display name:', err)
      toasts.add({ title: t('profile.displayNameUpdateFailed'), variant: 'error' })
    }
  }
  ...
  const handleAvatarUpload = async (file: File) => {
    if (!file.type.startsWith('image/')) {
      toasts.add({ title: t('profile.selectImageFile'), variant: 'error' })
      return
    }
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-2xl flex-col px-4 pb-16 sm:px-10">
      ...
      <div className="mt-6 flex flex-col gap-9">
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="divide-y divide-kumo-line overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
            ...
            <div className="flex items-center gap-4 px-5 py-4">
              ...
              >
                {displayAvatarUrl ? (
                  <img src={displayAvatarUrl} alt={t('profile.avatar')} className="h-full w-full object-cover" />
                ) : (
        ...
        <section aria-label={t('language.label')} className="rounded-xl border border-kumo-line bg-kumo-base p-5">
          <LanguageSelector className="max-w-sm" />
