    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          <div className="min-w-0">
            <Dialog.Title className="text-[15px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
              {t('misc.autoApprove.title', { action: actionLabel })}
            </Dialog.Title>
            <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {t('misc.autoApprove.description', { action: actionLabel, resource: resourceTitle })}
            </Dialog.Description>
          </div>
          <Dialog.Close
            render={(props) => (
              <WorkshopIconButton
                ...
                aria-label={t('misc.autoApprove.close')}
              >
                <X size={16} />
              </WorkshopIconButton>
            )}
        ...
        <div className="flex items-center justify-end gap-2 border-t border-kumo-line bg-kumo-base px-5 py-3">
          ...
          >
            {isProcessing ? t('misc.autoApprove.enabling') : t('misc.autoApprove.alwaysApprove')}
          </WorkshopButton>
        </div>
      </Dialog>
    </Dialog.Root>
