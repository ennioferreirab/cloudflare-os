...
export default function ConnectConnectorModal({
  ...
  const accountDisplayName =
    ...
    t('misc.connectConnector.connected')

  const headerTitle = isManage
    ? vendorDescription.displayName
    : t('misc.connectConnector.connect', { name: vendorDescription.displayName })
      ...
      <span className="truncate">
        {credentialsValid
          ? accountDescription?.uniqueName
            ...
            : accountDisplayName
          : t('misc.connectConnector.credentialsExpired')}
      </span>
    </div>
  ) : (
    ...
    >
      ...
      >
        <div className="shrink-0 flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          ...
          <Dialog.Close
            render={(props) => (
              <WorkshopIconButton {...props} disabled={busy} aria-label={t('misc.connectConnector.close')}>
                <X size={16} />
              </WorkshopIconButton>
            )}
          />
        ...
        <div className="new-gatekeeper-scroll-balanced min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          {supportedResources.length > 0 && (
            <div className="mt-5">
              <h3 className="mb-2 text-[12px] leading-4 font-semibold uppercase tracking-[0.6px] text-kumo-inactive">
                {granular
                  ? isManage
                    ? t('misc.connectConnector.resources')
                    : t('misc.connectConnector.resourcesToEnable')
                  : t('misc.connectConnector.capabilities')}
              </h3>
                    ...
                    >
                      ...
                      {grantable && (
                        <Switch
                          ...
                          aria-label={
                            isManage
                              ? t('misc.connectConnector.grantResource', { title: resource.title })
                              : t('misc.connectConnector.enableResource', { title: resource.title })
                          }
                          checked={checked}
                          disabled={disabled}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-default">
                  <span className="font-medium">
                    {t('misc.connectConnector.gatekeeperIntroTitle', { name: vendorDescription.displayName })}
                  </span>{' '}
                  <span className="text-kumo-subtle">
                    {t('misc.connectConnector.gatekeeperIntroDescription')}
                  </span>
                </div>
              </div>
            </div>
          ...
          {isManage && (
            <div className="mt-5 rounded-lg border border-kumo-line bg-kumo-elevated px-4 py-3 text-[12px] leading-[17px] font-normal tracking-[-0.2px] text-kumo-subtle">
              {t('misc.connectConnector.manageDescription')}
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center justify-between gap-3 border-t border-kumo-line bg-kumo-base px-5 py-3">
          {isManage && confirmingDisconnect ? (
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-default">
              {t('misc.connectConnector.disconnectQuestion', { name: vendorDescription.displayName })}
            </p>
          ) : isManage && hasPending ? (
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {pendingPatterns.length === 1
                ? t('misc.connectConnector.resourcesToAddOne')
                : t('misc.connectConnector.resourcesToAddMany', { count: pendingPatterns.length })}
            </p>
          ) : !isManage && granular && noneSelected ? (
            <p className="m-0 min-w-0 flex-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {t('misc.connectConnector.selectResource')}
            </p>
          ) : (
            <span aria-hidden />
          )}
            ...
            {isManage ? (
                ...
                {confirmingDisconnect ? (
                  <>
                    <WorkshopButton
                      ...
                      className="!h-9"
                    >
                      {t('misc.connectConnector.cancel')}
                    </WorkshopButton>
                    <WorkshopButton
                      tone="danger"
                    ...
                    >
                      {disconnecting ? t('misc.connectConnector.disconnecting') : t('misc.connectConnector.yesDisconnect')}
                    </WorkshopButton>
                  </>
                ) : hasPending ? (
                  <>
                    ...
                    >
                      {ensuringBusy
                        ? t('misc.connectConnector.opening')
                        : t('misc.connectConnector.continueTo', { name: vendorDescription.displayName })}
                    </WorkshopButton>
                  </>
                ) : (
                  <>
                    ...
                    <WorkshopButton
                      ...
                      className="!h-9"
                    >
                      {t('misc.connectConnector.disconnect')}
                    </WorkshopButton>
                  </>
                )}
              </>
              ...
              <>
                ...
                >
                  {autoProvisions
                    ? connecting
                      ? t('misc.connectConnector.adding')
                      : t('misc.connectConnector.add', { name: vendorDescription.displayName })
                    : connecting
                    ? t('misc.connectConnector.opening')
                    : t('misc.connectConnector.continueTo', { name: vendorDescription.displayName })}
                </WorkshopButton>
